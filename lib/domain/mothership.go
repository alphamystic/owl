...
func (d *Domain) CreateMothership(m ent.Mothership,ctx context.Context) error {
...
func (d *Domain) ListMotherShips(active bool,ctx context.Context) ([]ent.Mothership,error) {
  conn,err := d.GetConnection(ctx)
  if err != nil {
  ...
  defer rows.Close()
  var msps []ent.Mothership
  for rows.Next(){
    var m ent.Mothership
...
func (d *Domain) ViewMothership(msid string,ctx context.Context)(*ent.Mothership,error){
  conn,err := d.GetConnection(ctx)
  if err != nil {
    return nil,fmt.Errorf("Error getting db connection: %q",err)
  }
