...
func (d *Doamin) CreateAsset(a ent.Asset,ctx context.Context) error {
...
func (d *Doamin)  ListAssets(active bool,ctx context.Context)([]ent.Asset,error){
  conn,err := d.GetConnection(ctx)
  if err != nil {
...
func (d *Doamin)  ViewAsset(assetId string,ctx context.Context)(*ent.Asset,error){
  conn,err := d.GetConnection(ctx)
  if err != nil {
    return nil,fmt.Errorf("Error getting db connection: %q",err)
  }
